...
assert(decodedCursor.id === record.id && decodedCursor.createdAt === record.created_at, "collection cursor should preserve its timestamp and UUID");
const preciseCursor = handler._test.decodeCursor(handler._test.encodeCursor({ ...record, created_at: "2026-07-01T10:00:00.123456+00:00" }));
assert(preciseCursor.createdAt === "2026-07-01T10:00:00.123456+00:00", "collection cursor must preserve PostgreSQL microseconds exactly");
